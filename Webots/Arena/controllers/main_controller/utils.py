...
def store_block(self, pos, range=0.1, fName='vision.json'):
    ...
    for block in data["blocks"]:
        ...
        if np.linalg.norm(block_pos - pos) < range:
            return False
    data["blocks"].append(pos)
    json.dump(data, open(fName, 'w+'))
    print(f"block stored at {pos}")
    return True

def get_next_block_pos(self, my_pos, fName='vision.json'):
    ...
    blocks_dist = np.linalg.norm(blocks-pos)
